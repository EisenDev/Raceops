      ...
      <div className="flex flex-col lg:flex-row">
        <div className="p-6 flex-1 space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="font-black text-[#1A1A1A] uppercase tracking-tight">{request.team.name}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-[#F9F9F9] p-4 rounded-xl border border-[#1A1A1A]/5 space-y-2">
              <p className="text-[10px] font-black uppercase tracking-widest text-[#999999]">Current Score</p>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-black text-[#666666]">{currentValue.totalPoints}</span>
                <span className="text-[10px] font-bold text-[#999999]">PTS</span>
              </div>
              <p className="text-[10px] font-bold text-[#999999] uppercase tracking-tighter">{currentValue.scoringMode.replace('_', ' ')}</p>
            </div>
            ...
            <div className="bg-white p-4 rounded-xl border-2 border-[#1A1A1A] shadow-md space-y-2 relative overflow-hidden">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-[#1A1A1A]">Requested Score</p>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-black text-[#1A1A1A]">{requestedValue.totalPoints}</span>
                <span className="text-[10px] font-bold text-[#1A1A1A]">PTS</span>
              </div>
              <p className="text-[10px] font-bold text-[#666666] uppercase tracking-tighter">{requestedValue.scoringMode.replace('_', ' ')}</p>
            </div>
          </div>

          <div className="space-y-1">
            <p className="text-[10px] font-black uppercase tracking-widest text-[#999999]">Reason for correction</p>
          ...
          {request.adminRemarks && (
            <div className="space-y-1 pt-2 border-t border-[#1A1A1A]/5">
              <p className="text-[10px] font-black uppercase tracking-widest text-[#999999]">Admin Remarks</p>
        ...
        {isAdmin && request.status === 'PENDING' && (
          <div className="bg-[#F9F9F9]/50 lg:border-l border-[#1A1A1A]/5 p-6 flex flex-col justify-center gap-4 lg:w-72">
            <p className="text-[10px] font-black uppercase text-center text-[#999999]">Admin Actions</p>
            
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-[#1A1A1A]">Internal Remarks</label>
              <textarea 
            ...
            <div className="flex flex-col gap-2">
              <Button onClick={handleApprove} disabled={isProcessing} className="w-full font-bold text-xs h-10">
                <Check size={14} className="mr-2" />
                Approve & Apply
              </Button>
              <Button onClick={handleDecline} disabled={isProcessing} variant="outline" className="w-full font-bold text-xs h-10 text-red-600 hover:bg-red-50 hover:border-red-100">
